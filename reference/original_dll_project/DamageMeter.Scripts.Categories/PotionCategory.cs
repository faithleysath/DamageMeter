...
public class PotionCategory : IStatCategory
{
	...
	public List<BarData> GetDetailBars(string playerKey)
	{
		...
		List<BarData> val = new List<BarData>();
		global::System.Collections.Generic.IEnumerator<KeyValuePair<string, int>> enumerator = ((global::System.Collections.Generic.IEnumerable<KeyValuePair<string, int>>)Enumerable.OrderByDescending<KeyValuePair<string, int>, int>((global::System.Collections.Generic.IEnumerable<KeyValuePair<string, int>>)playerStats.PotionUseCount, (Func<KeyValuePair<string, int>, int>)((KeyValuePair<string, int> k) => k.Value))).GetEnumerator();
		try
		{
			while (((global::System.Collections.IEnumerator)enumerator).MoveNext())
			{
				...
				{
					Key = current.Key,
					Label = CombatDataCollector.ResolvePotionName(current.Key),
					Value = current.Value
				});
			}
		}
